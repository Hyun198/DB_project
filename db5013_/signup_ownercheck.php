...
<?php
  ...
  if ( !is_null( $username ) ) {
    $jb_conn = oci_connect($dbusername,$dbpassword,$db);
      ...
      $jb_sql = "SELECT REST_ID FROM RESTAURANT WHERE REST_ID = '$username'";
      ...
      if ( $username == $username_e ) {//동일한 유저이름이 있을경우
        ...
        $jb_sql = "INSERT INTO RESTAURANT ( REST_ID, REST_PW ,RESTAURANT_NAME,AREA_CODE_R,PHONE_NUM_R) VALUES ( '$username', '$password','$restid','$address' ,'$pnum')";
        $jb_result =oci_parse( $jb_conn, $jb_sql );
        oci_execute($jb_result);
        header( 'Location: register_ok.php' );
      }
    
      echo "<script>location.replace('signup_owner.php?wp=$wp&wu=$wu');</script>"; 
  }
